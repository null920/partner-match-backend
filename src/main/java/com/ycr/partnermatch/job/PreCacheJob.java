...
public class PreCacheJob {
    ...
    public void doCacheRecommendUser() {
        int pageNum = 1;
        RLock lock = redissonClient.getLock(String.format("partnerMatch:recommend:pageNum:%s", pageNum));
        try {
            // 只有一个线程能获得锁
            if (lock.tryLock(0, 30000L, TimeUnit.MILLISECONDS)) {
                ...
                for (Long userId : mainUserList) {
                    ...
                    Page<User> userPage = userService.page(new Page<>(pageNum, 20), wrapper);
